...
////////////////////////////////////////////////////////////////////////////////
if (!function_exists('browscap_incparse'))
{
  function browscap_incparse($root,$source,$parsemode,$return=false)
  {
    ...
    switch ($parsemode)
    {
      ...
      $html = browscap_textileparser($content,true);
      break;
      ...
      case 'php':
      $source = $root.$source;
      $html =  browscap_include_contents($source);
      break;
...
////////////////////////////////////////////////////////////////////////////////
if (!function_exists('browscap_textileparser'))
{
  function browscap_textileparser($textile,$return=false)
  {
...
////////////////////////////////////////////////////////////////////////////////
if (!function_exists('browscap_echotextile'))
{
  function browscap_echotextile($msg) {
    global $REX;
    if(OOAddon::isAvailable("textile")) {
      if($msg!='') {
...
////////////////////////////////////////////////////////////////////////////////
if (!function_exists('browscap_include_contents'))
{
  function browscap_include_contents($filename) {
    if (is_file($filename)) {
      ob_start();
      include $filename;
...
/**
  * browscap_scandir Funktion - Recursiver Scan eines Verzeichnisses
  *
  ...
  *                               )
  */
if (!function_exists('browscap_scandir'))
{
  function browscap_scandir($source, $limit=0, $blacklist=array(), $whitelist=array(), &$result=array())
  {
    ...
    foreach($dirscan as $item)
    {
      ...
      if (is_dir($source.$item))
      {
        ...
        if($limit && intval($limit))
        {
          if($limit > $depth)
          {
            browscap_scandir($source.$item.'/', $limit, $blacklist, $whitelist, $result);
          }
        }
        else
        {
...
// SAVE ADDON SETTINGS
////////////////////////////////////////////////////////////////////////////////
if (!function_exists('browscap_saveConf'))
{
  function browscap_saveConf($myCONF)
  {
    global $REX,$mypage;
...
////////////////////////////////////////////////////////////////////////////////
if (!function_exists('browscap_nl_2_array'))
{
  function browscap_nl_2_array($str)
  {
...
if (!function_exists('browscap_array_2_nl'))
{
  function browscap_array_2_nl($arr)
  {
    return count($arr)>0 ? implode(PHP_EOL,$arr) : '';
  }
}

if (!function_exists('browscap_cast'))
{
  function browscap_cast($request,$conf)
  {
    if(is_array($request) && is_array($conf))
    {
      foreach($conf as $key => $cast)
      {
        switch($cast)
        {
          ...
          case 'nl_2_array':
            $request[$key] = browscap_nl_2_array($request[$key]);
            break;
